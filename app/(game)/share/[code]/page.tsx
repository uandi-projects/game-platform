  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        description,
        type: "website",
        url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://uandi-game-platform.vercel.app'}/share/${gameCode}`,
    ...
    return {
      title: "Game | U&I Game Platform",
      description: "Join a game on U&I Game Platform!",
    };
  }
}

export default async function ShareGamePage({ params }: { params: Promise<{ code: string }> }) {
  const resolvedParams = await params;
  const gameCode = resolvedParams.code.toUpperCase();

  // Redirect to the existing game joining flow
